...
def program_feb(regs, addr, firmware, port='/dev/ttyPS1', baud='115200', modbus_addr='\x14') -> None:
    do_write(0, 0, regs)
    time.sleep(2)
    do_write(1, 0, regs)
    time.sleep(2)
    ...
    print(f'Executing stm32flash -b {baud} -w {firmware} -v {port}')
    subprocess.run(['stm32flash',  '-b', f'{baud}', '-w', f'{firmware}',
                    '-v', f'{port}'], shell=False)
    print(f'Executing echo -n -e {modbus_addr} | stm32flash -b {baud} -w - -S 0x08080000:1 -v {port}')
    first_command = subprocess.Popen(['echo', '-n', '-e', f'{modbus_addr}'], stdout=subprocess.PIPE, text=True)
    #second_command = subprocess.Popen(['hexdump', '-C'], stdin=first_command.stdout, stdout=subprocess.PIPE, text=True)
    second_command = subprocess.Popen(['stm32flash', '-b', f'{baud}', '-w', '-', '-S 0x08080000:1'
                    '-v', f'{port}'], stdin=first_command.stdout, stdout=subprocess.PIPE, text=True)
    output_second_command = second_command.communicate()[0] 
    print(output_second_command)
...
def main():
    ...
    modbus_addrs = {1:'\x15', 4:'\x16', 7:'\x17', 10:'\x18', 19:'\x19'}
    ...
    for i in febnum:
        program_feb(registers, int(i)-1, parser_args.filename, parser_args.port, parser_args.baud, modbus_addrs[int(i)])
