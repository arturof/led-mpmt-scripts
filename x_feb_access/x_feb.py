# Version 2.0 - R. Gornea on October 23, 2025 - changed the main board register mapping following the v3 release on 23.10.2025
# Version 1.1 - R. Gornea on October 8, 2025 - upgrade partially based on Rolf S. updates, added support for pymodbus 3.9.2
...
# MODBUS address allocation - define from lowest to highest port on the main board
...
# LED-FEB firmware constants
SELECT_MAIN_BOARD = 0x0
SELECT_uCONTROLLER = 0x1
SELECT_EXT_CRYSTAL = 0x2
# main board register mapping
...
class xFEB_Modbus:
    def __init__(self, debug_modbus_addr = 20):
        ...
        if (os.path.exists("/dev/uio0") == True):
            ...
            print(f"[WARNING]: Running with a single LED-FEB board at MODBUS address {debug_modbus_addr}")
            self.n_led_febs = 1
    ...
    # low level function call
    def get_discretes(self, modbus_id, start_addr, my_count):
        if (self.connected):
            if (self.run_mode == False):
                if (self.led_febs_array[0] != modbus_id):
                    print(f"[WARNING]: Ignored requested MODBUS ID {modbus_id} and used opened device ID {self.led_febs_array[0]}")
    ...
    def get_discrete(self, modbus_id, coil_addr):
        temp_read = self.get_discretes(modbus_id, coil_addr, 1)
        if (temp_read == None):
            return temp_read
        else:
            return temp_read.bits[0]
    ...
    def set_led_bias(self, led_feb_id, level_in_volts):
        if (led_feb_id < self.get_n_led_febs()):
            dac_level = int(4711.9 - 307.97 * level_in_volts)
            if dac_level > 4095:
                dac_level = 4095
            elif dac_level < 0:
                dac_level = 0
    ...
    def set_max_led_bias(self, led_feb_id):
        return self.set_led_bias(led_feb_id, 15.28)

    # high level function call
    def set_min_led_bias(self, led_feb_id):
        return self.set_led_bias(led_feb_id, 2.02)
    ...
    def fetch_led_bias(self, led_feb_id):
        ...
        if (value == None):
            ...
            return "LED bias voltage is at minimum"
        elif (value > 15.25):
            return "LED bias voltage is at maximum"
        else:
            return f"LED bias voltage is {value:.2f} volts"
    ...
    def fetch_i_mon(self, led_feb_id):
        ...
        if (value == None):
            ...
            return f"LED bias current is {value:.3f} mA"  
